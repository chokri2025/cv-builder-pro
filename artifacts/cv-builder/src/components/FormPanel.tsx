    ...
    <div className="form-panel">
      <div className="form-header">
        <div className="form-logo">
          ...
          <span>
            {t('nav.logo')} <span className="pro-badge">{t('nav.pro')}</span>
          </span>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <button
            className="reset-btn"
            onClick={() => {
              if (window.confirm(t('builder.resetConfirm'))) onClear();
            }}
      ...
      <div className="form-sections">
        ...
        >
          <div className="photo-upload-area" onClick={() => fileInputRef.current?.click()}>
            {cvData.personal.profilePicture ? (
              <img
                src={cvData.personal.profilePicture}
                alt={
                  cvData.personal.fullName
                    ? `${cvData.personal.fullName}'s profile photo`
                    : 'Profile photo'
                }
        ...
        <CollapsibleSection
          title={`${t('sections.experience')} (${cvData.experience.length})`}
        ...
        <CollapsibleSection
          title={`${t('sections.education')} (${cvData.education.length})`}
        ...
        <CollapsibleSection
          title={`${t('sections.skills')} (${cvData.skills.length})`}
        ...
        <CollapsibleSection
          title={`${t('sections.languages')} (${cvData.languages.length})`}
        ...
        >
          {cvData.languages.map((lang) => (
            <div key={lang.id} className="repeatable-item lang-item">
              ...
              <button className="remove-btn-inline" onClick={() => removeLanguage(lang.id)}>
                ✕
              </button>
            </div>
          ))}
        ...
        <CollapsibleSection
          title={`${t('sections.projects')} (${cvData.projects.length})`}
